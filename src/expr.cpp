...
template <> void Stack<llvm::Value *>::dump(std::ostream &out) const {
    int n = 0;
    for (auto s : stack) {
...
static llvm::Value *TempStringFromStringExpr(llvm::Value *dest, StringExprAST *rhs) {
    ...
    return builder.CreateMemCpy(dest2, v, rhs->Str().size(),
...
llvm::Value *NilExprAST::CodeGen() {
    ...
    return llvm::ConstantPointerNull::get(
        llvm::dyn_cast<llvm::PointerType>(Types::GetVoidPtrType()));
}
...
ArrayExprAST::ArrayExprAST(const Location &loc, VariableExprAST *v,
...
llvm::Value *BinaryExprAST::CallSetFunc(const std::string &name, bool resTyIsSet) {
    ...
    if (resTyIsSet) {
        llvm::Constant *f =
            GetFunction(Types::GetVoidType(), {pty, pty, pty, intTy}, "__Set" + name);
...
static llvm::Value *CallStrFunc(const std::string &name, ExprAST *lhs, ExprAST *rhs,
                                Types::TypeDecl *resTy, const std::string &twine) {
    TRACE();
...
static llvm::Value *CallStrCat(ExprAST *lhs, ExprAST *rhs) {
    ...
    llvm::Constant *f =
        GetFunction(Types::GetVoidType(), {pty, lV->getType(), rV->getType()}, "__StrConcat");
...
llvm::Function *FunctionAST::CodeGen(const std::string &namePrefix) {
    ...
    if (debugInfo) {
        ...
        llvm::DISubprogram *    sp =
            di.builder->createFunction(fnContext, name, "", unit, lineNum, st, false, true,
                                       lineNum, llvm::DINode::FlagPrototyped, false);
    ...
    if (VariableExprAST *rhsv = llvm::dyn_cast<VariableExprAST>(rhs)) {
        if (rhsv->Type() == lhsv->Type()) {
            ...
            if (!disableMemcpyOpt && size >= MEMCPY_THRESHOLD) {
                ...
                return builder.CreateMemCpy(dest, src, size,
                                            std::max(rhsv->Type()->AlignSize(), MIN_ALIGN));
            }
        }
    }
        ...
        if (!func) {
            ...
            if (cd && cd->VTableType(true)) {
                llvm::GlobalVariable *gv =
                    theModule->getGlobalVariable("vtable_" + cd->Name(), true);
            ...
            llvm::GlobalValue::LinkageTypes linkage =
                (var.IsExternal() ? llvm::GlobalValue::ExternalLinkage
                                  : llvm::Function::InternalLinkage);

            llvm::GlobalVariable *gv =
                new llvm::GlobalVariable(*theModule, ty, false, linkage, init, var.Name());
            const llvm::DataLayout dl(theModule);
            ...
            if (debugInfo) {
                ...
                }
                {
                    ...
                    di.builder->createGlobalVariableExpression(scope, var.Name(), var.Name(), unit,
                                                               lineNum, debugType,
            ...
            if (debugInfo) {
                ...
                }
                {
                    ...
                    int                    lineNum = this->Loc().LineNumber();
                    llvm::DIFile *         unit = sp->getFile();
                    llvm::DILocalVariable *dv = di.builder->createAutoVariable(
                        sp, var.Name(), unit, lineNum, debugType, true);
...
llvm::Value *SetExprAST::MakeConstantSet(Types::TypeDecl *type) {
    ...
    llvm::Constant *init =
        llvm::ConstantArray::get(aty, llvm::ArrayRef<llvm::Constant *>(initArr, size));
    llvm::GlobalValue::LinkageTypes linkage = llvm::Function::InternalLinkage;
    std::string                     name("P" + std::to_string(index) + ".set");
    llvm::GlobalVariable *          gv =
        new llvm::GlobalVariable(*theModule, ty, false, linkage, init, name);
...
llvm::Value *SetExprAST::Address() {
    ...
    for (auto v : values) {
        ...
        if (RangeExprAST *r = llvm::dyn_cast<RangeExprAST>(v)) {
            ...
            loop = builder.CreateAdd(
                loop, MakeConstant(1, llvm::dyn_cast<Types::SetDecl>(type)->SubType()), "update");
...
llvm::Value *RangeReduceAST::CodeGen() {
    ...
    if (ty->getPrimitiveSizeInBits() <
        Types::GetIntegerType()->LlvmType()->getPrimitiveSizeInBits()) {
...
llvm::Value *RangeCheckAST::CodeGen() {
    ...
    std::vector<llvm::Value *> args = {
        builder.CreateGlobalStringPtr(Loc().FileName()), MakeIntegerConstant(Loc().LineNumber()),
        MakeIntegerConstant(start), MakeIntegerConstant(end), orig_index};
    std::vector<llvm::Type *> argTypes = {
        llvm::PointerType::getUnqual(Types::GetCharType()->LlvmType()), intTy, intTy, intTy,
        intTy};
...
llvm::Value *VTableAST::CodeGen() {
    ...
    vtable = new llvm::GlobalVariable(*theModule, ty, true, llvm::Function::InternalLinkage, init,
                                      name);
    if (!init) {
...
VirtFunctionAST::VirtFunctionAST(const Location &w, VariableExprAST *slf, int idx,
                                 Types::TypeDecl *ty)
...
llvm::Value *TrampolineAST::CodeGen() {
    ...
    llvm::Constant *llvmTramp =
        GetFunction(Types::GetVoidType()->LlvmType(), {voidPtrTy, voidPtrTy, voidPtrTy},
