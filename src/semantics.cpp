...
Types::TypeDecl *TypeCheckVisitor::BinarySetUpdate(BinaryExprAST *b) {
    ...
    if (*lty->SubType() != *rty->SubType()) {
        ...
        Types::Range *range =
            new Types::Range(std::min(lr->Start(), rr->Start()), std::max(lr->End(), rr->End()));
...
void TypeCheckVisitor::CheckCallExpr(CallExprAST *c) {
    ...
    for (auto &a : c->args) {
        ...
        if (const Types::TypeDecl *ty = parg[idx].Type()->CompatibleType(a->Type())) {
            ...
            bad = false;
        } else if (Types::FuncPtrDecl *argTy =
                       llvm::dyn_cast<Types::FuncPtrDecl>(parg[idx].Type())) {
    ...
    if (isText) {
        for (auto arg : r->args) {
            ...
            if (arg->Type()->IsCompound()) {
                ...
                if (bad) {
                    Error(arg,
                          "Read argument must be simple type [or array of char NYI] or string");
                }
            }
        }
    } else {
