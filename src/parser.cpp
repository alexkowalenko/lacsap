...
int64_t Parser::ParseConstantValue(Token::TokenType &tt, Types::TypeDecl *&type) {
    ...
    case Token::Identifier: {
        ...
        if (const EnumDef *ed = GetEnumValue(CurrentToken().GetIdentName())) {
            ...
            tt = Token::Unknown;
            return ErrorI(CurrentToken(),
                          "Invalid constant, expected identifier for enumerated type");
        }
        break;
    }
    default:
        tt = Token::Unknown;
        return ErrorI(CurrentToken(),
                      "Invalid constant value, expected char, integer or enum value");
    }
...
Types::RangeDecl *Parser::ParseRangeOrTypeRange(Types::TypeDecl *&type) {
    if (CurrentToken().GetToken() == Token::Identifier) {
        if ((type = GetTypeDecl(CurrentToken().GetIdentName()))) {
            if (!type->IsIntegral()) {
                return ErrorR(CurrentToken(),
                              "Type used as index specification should be integral type");
            }
            AssertToken(Token::Identifier);
    ...
    if (ParseCommaList(ccv, Token::RightParen, false)) {
        ...
        for (auto v : ty->Values()) {
            if (!nameStack.Add(v.name, new EnumDef(v.name, v.value, ty))) {
                return reinterpret_cast<Types::EnumDecl *>(Error(
                    CurrentToken(), "Enumerated value by name " + v.name + " already exists..."));
            }
        }
        return ty;
    }
...
Types::PointerDecl *Parser::ParsePointerType(bool maybeForwarded) {
    assert(
        (CurrentToken().GetToken() == Token::Uparrow || CurrentToken().GetToken() == Token::At) &&
        "Expected @ or ^ token...");
    NextToken();
    ...
    do {
        ...
        for (auto i = b; i != e; i++) {
            ...
            if (f != i && f != e) {
                // TODO: Track location.
                return reinterpret_cast<Types::VariantDecl *>(
                    Error(CurrentToken(), "Value already used: " + std::to_string(*i) +
                                              " in variant declaration"));
            }
        }
        if (!Expect(Token::LeftParen, true)) {
            return 0;
...
Types::ClassDecl *Parser::ParseClassDecl(const std::string &name) {
    ...
    if (AcceptToken(Token::LeftParen)) {
        ...
        if (!(base = llvm::dyn_cast_or_null<Types::ClassDecl>(GetTypeDecl(baseName)))) {
            return reinterpret_cast<Types::ClassDecl *>(
                Error(CurrentToken(), "Expected class as base"));
        }
        AssertToken(Token::Identifier);
        if (!Expect(Token::RightParen, true)) {
            return 0;
...
Types::TypeDecl *Parser::ParseType(const std::string &name, bool maybeForwarded) {
    ...
    if (tt == Token::Packed) {
        ...
        if (tt != Token::Array && tt != Token::Record && tt != Token::Set && tt != Token::File) {
            return ErrorT(CurrentToken(),
                          "Expected 'array', 'record', 'file' or 'set' after 'packed'");
        }
    }
...
ExprAST *Parser::ParseStringExpr(Token token) {
    ...
    std::vector<Types::RangeDecl *> rv = {
        new Types::RangeDecl(new Types::Range(0, len), Types::GetIntegerType())};
...
ExprAST *Parser::ParseUnaryOp() {
    assert((CurrentToken().GetToken() == Token::Minus ||
            CurrentToken().GetToken() == Token::Plus || CurrentToken().GetToken() == Token::Not) &&
...
ExprAST *Parser::MakeSimpleCall(ExprAST *expr, const PrototypeAST *proto,
...
ExprAST *Parser::MakeSelfCall(VariableExprAST *self, Types::MemberFuncDecl *mf,
                              Types::ClassDecl *cd, std::vector<ExprAST *> &args) {
    ExprAST *           expr = 0;
    ...
    if (Expect(Token::Identifier, false)) {
        ...
        if (Types::ClassDecl *cd = llvm::dyn_cast<Types::ClassDecl>(type)) {
            ...
            return ErrorV(CurrentToken(),
                          "Attempt to use field of variable that hasn't got fields");
        }
        if (!e && v) {
...
bool Parser::ParseArgs(const NamedObject *def, std::vector<ExprAST *> &args) {
    ...
    if (AcceptToken(Token::LeftParen)) {
        ...
        if (const FuncDef *funcDef = llvm::dyn_cast_or_null<FuncDef>(def)) {
            ...
            if (const Types::FuncPtrDecl *fp =
                    llvm::dyn_cast<Types::FuncPtrDecl>(varDef->Type())) {
                proto = fp->Proto();
            }
        }
        while (!AcceptToken(Token::RightParen)) {
                    ...
                    if (const NamedObject *argDef = nameStack.Find(idName)) {
                        if (const FuncDef *fd = llvm::dyn_cast<FuncDef>(argDef)) {
                            ...
                            if (vd->Type()->Type() == Types::TypeDecl::TK_FuncPtr) {
                                arg = new VariableExprAST(CurrentToken().Loc(), idName,
                                                          argDef->Type());
                            }
                        }
                    }
...
ExprAST *Parser::ParseVariableExpr(const NamedObject *def) {
    ...
    Token::TokenType tt = CurrentToken().GetToken();
    while (tt == Token::LeftSquare || tt == Token::Uparrow || tt == Token::At ||
           tt == Token::Period) {
        ...
        if (AcceptToken(Token::Period)) {
            if (Types::TypeDecl *ty = GetTypeDecl(funcName)) {
                if ((od = llvm::dyn_cast<Types::ClassDecl>(ty))) {
                    ...
                    if (!membfunc) {
                        return ErrorP(CurrentToken(), "Member function '" + m +
    ...
    while (!AcceptToken(Token::End)) {
        ...
        if (CurrentToken().GetToken() == Token::Integer &&
            ...
            if (!nameStack.FindTopLevel(std::to_string(n))) {
                ...
                return reinterpret_cast<BlockAST *>(Error(
                    CurrentToken(), "Can't use label in a different scope than the declaration"));
            }
...
FunctionAST *Parser::ParseDefinition(int level) {
    ...
    if (!(fnDef && fnDef->Proto() && fnDef->Proto() == proto)) {
        ...
        if (Types::ClassDecl *cd = proto->BaseObj()) {
            ...
            if (elem < 0) {
                return ErrorF(CurrentToken(), "Name '" + shortname +
                                                  "' doesn't appear to be a member function...");
            }
    ...
    if (Expect(Token::Assign, true)) {
        if (ExprAST *start = ParseExpression()) {
            ...
            if (tt == Token::Downto || tt == Token::To) {
                ...
                return Error(CurrentToken(),
                             "Expected 'to' or 'downto', got " + CurrentToken().ToString());
            }
...
ExprAST *Parser::ParseCaseExpr() {
    ...
    do {
        ...
        if (CurrentToken().GetToken() != Token::Else &&
            CurrentToken().GetToken() != Token::Otherwise) {
            NextToken();
        }
        switch (CurrentToken().GetToken()) {
        case Token::Comma:
        ...
        case Token::Else: {
            ...
            if (isOtherwise) {
                ...
                if (lab.size()) {
                    return Error(CurrentToken(),
                                 "Can't have multiple case labels with 'otherwise' "
                                 "or 'else' case label");
                }
            } else {
...
void Parser::ExpandWithNames(const Types::FieldCollection *fields, VariableExprAST *v,
                             int parentCount) {
    TRACE();
    int vtableoffset = 0;
    ...
    for (int i = 0; i < fields->FieldCount(); i++) {
        ...
        if (f->Name() == "") {
            ...
            VariableExprAST *vv =
                new VariantFieldExprAST(CurrentToken().Loc(), v, parentCount, ty);
    ...
    if (Types::ClassDecl *od =
            const_cast<Types::ClassDecl *>(llvm::dyn_cast<Types::ClassDecl>(fields))) {
...
class CCWith : public Parser::CommaConsumer {
  public:
    CCWith(Stack<const NamedObject *> &ns) : levels(0), nameStack(ns) {}
    bool Consume(Parser &parser) override {
        nameStack.NewLevel();
        levels++;
        if (parser.Expect(Token::Identifier, false)) {
...
ExprAST *Parser::ParseRead() {
    ...
    assert(
        (CurrentToken().GetToken() == Token::Read || CurrentToken().GetToken() == Token::Readln) &&
        ...
        case Token::Function: {
            ...
            if (!nameStack.Add(name, nmObj)) {
                return ErrorF(CurrentToken(),
                              "Interface name '" + name + "' already exists in...");
            }
        } break;
...
ExprAST *Parser::ParseUnit(ParserType type) {
    ...
    do {
        ...
        case Token::Begin: {
            ...
            PrototypeAST *proto =
                new PrototypeAST(loc, initName, std::vector<VarDef>(), Types::GetVoidType(), 0);
