...
llvm::DIType *RecordDecl::GetDIType(llvm::DIBuilder *builder) const {
    ...
    return builder->createStructType(scope, name, unit, lineNo, size, align,
                                     llvm::DINode::FlagZero, derivedFrom, elements);
}
...
llvm::Type *ClassDecl::GetLlvmType() const {
    ...
    for (int i = 0; i < fc; i++) {
        ...
        assert(!llvm::isa<MemberFuncDecl>(f->FieldType()) &&
               "Should not have member functions now");
