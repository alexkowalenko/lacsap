...
std::string compilers[] = {"../bin/lacsap", "fpc -Mdelphi"};
...
bool TimeTestCase::Result() {
    ...
    if (elapsed > maxTime) {
        std::cerr << "Took too long to compile  " << source << " " << std::fixed
...
CompileTimeError::CompileTimeError(const std::string &nm, const std::string &src,
...
TestCase *TestCaseFactory(const std::string &type, const std::string &name,
                          const std::string &source, const std::string &args) {
    if (type == "File") {
